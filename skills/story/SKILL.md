...
| 触发条件 | 处理 → 兜底 |
|---|---|
| 命中多行 / 多意图 | 拆成有序步骤或取最强意图，复述确认 → 列选项让用户选 |
| 完全无法匹配 | 列出路由表选项让用户选 → 问具体想做什么 |
| 目标 skill 不可用 | 推荐最接近的替代 skill → 说明可用清单 |
| explorer/researcher 未部署 | 主线程直接读项目文件回答 → 提示 `/story-setup` |

## 不要做（反例黑名单）

- ❌ 意图模糊（未指明长/短篇）就替用户猜、直接进写作 skill
- ❌ 开放式提问"你想做什么"——要给路由表选项让用户选
- ❌ 项目未部署（无 `.story-deployed`）就直接进写作 skill，须先提示 `/story-setup`
- ❌ 把扫榜/拆文/封面需求误路由到写作 skill
- ❌ 在路由入口内直接写正文/拆文/扫榜（只分发，不执行）
- ❌ 多意图时静默只做一个
