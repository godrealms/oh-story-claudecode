...
所有从导入生成的文件标注 `[导入反推]` 标记，表示内容来自自动抽取。🔴 **CHECKPOINT**：带此标记的文件续写前必须由用户复核，不得直接当作可靠设定使用。
...
2. **用户确认**：
   ...
   - 目标平台：{起点/番茄/晋江/其他}
   - 是否完本：{是/否（半成品写到第N章）}
3. 🔴 **CHECKPOINT · 输出确认**：向用户展示检测到的章节范围和字数，**用户确认后**才开始分析
...
- ❌ 跳过深度分析直接迁移——先分析后迁移，拆文库数据保留
- ❌ 在未备份原文（`拆文库/{书名}/原文/`）时就开始分析
- ❌ 改写原文内容——正文迁移保留原文不变
- ❌ 把 `[导入反推]` 内容当可靠设定直接续写——续写前必须复核
- ❌ 为无法稳定反推的字段（章首/章尾钩子）编造内容——标 `[待补充]`
