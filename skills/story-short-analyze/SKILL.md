...
| 触发条件 | 一线处理 → 仍失败兜底 |
|---|---|
| 文本过短提不足节点 | 降采样提关键节点，不硬拆 → 报告标注「文本过短，节点降采样」 |
| 非标体例（聊天/书信/帖子） | 按时间/说话人/信息揭示点分段 → 映射到开端/发展/高潮/结局 |
| 反转不明显 | 检查是否靠共鸣/余韵驱动 → 标注「非反转驱动型」并按驱动力分析 |
| 质量门控字段缺失 | 回对应阶段补齐 → 标注缺失项，不带病进下一阶段 |

## 不要做（反例黑名单）

- ❌ 为凑节点数量硬拆短文本——文本短就降采样提取关键节点
- ❌ 机械按自然段分段——非标体例按时间/说话人/信息揭示点分
- ❌ 把"有反转"当唯一价值，忽略共鸣层次与余韵
- ❌ 跳过质量门控必填字段
- ❌ 把拆文写成剧情复述，而非结构与手法分析
