...
- 删除比例上限按 AI 味等级分级：轻度 ≤15%，中度 ≤25%，重度 ≤35%。重度文本可通过“合并重复描写+重写降AI”产生更大字符差，但仍不得整段删除或删掉剧情功能。🛑 **STOP（超限）**：超过对应比例时停止删除，在报告中标记超限风险并输出分段处理方案，不得继续删
- 如果逐句修改后某段仍不满意，在去AI味报告中标注 `[需复核]` 而非删除，不计入对应等级的删除比例上限
