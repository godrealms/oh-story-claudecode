...
🔴 **CHECKPOINT（必填项）**：书名、作者名（笔名）、目标平台为必填，缺任一项先停下问用户，不要用占位名直接生成。
...
🔴 **CHECKPOINT（出图确认）**：首次出图后停下让用户确认；不满意时调整方向——更换构图、调整色调、换字体风格、换平台风格，确认满意再定稿。
...
- ❌ 让作者名沦为模糊小字——必须有字体+颜色+装饰三件套
- ❌ 文字遮挡人物脸/核心画面
- ❌ 堆砌矛盾的多题材视觉——一眼只传达一个题材
- ❌ 必填信息（书名/作者/平台）缺失就用占位名直接出图
