...
> **日更模式准备步骤**：每章写作前执行 2 步（状态筛选 + 意图确认），嵌入 Step 2 逐章执行循环内。日更模式省略对标结构化子目录检索（SKILL.md 步骤 3.2 的第二部分），因为题材框架已在 `设定/题材定位.md` 中固化，对标模块已在 Phase 2-3 中内化到卷纲和细纲。情绪-技法匹配（步骤 3.2 的第一部分）通过 Step 2.2 的意图确认覆盖。完整 3 步准备层见 SKILL.md「单章写作流程」步骤 3。
