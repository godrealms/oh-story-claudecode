...
3. 如均不存在，提示用户：「对标书原文已放入 `对标/{书名}/原文/`。要先用 `/story-long-analyze` 深度拆解吗？拆完后 `拆文报告.md` 会自动存入 `拆文库/{书名}/`，写作时会自动按 `对标/ → 拆文库/` 顺序读取。」
...
🔴 **大纲锁定（CHECKPOINT）**：已进入正文写作的前 10 章细纲锁定，**未经用户确认不得修改**；后续滚动细纲可随正文反馈微调。
...
🛑 **字数硬门槛（STOP）**：默认最低字数 3000 字/章（细纲另有标注以细纲为准）。**低于最低字数禁止结束本章，必须补足后再继续。**
...
- ❌ 跳过细纲直接写正文——无细纲必须先补建
- ❌ 用大段心理独白凑字——内心变化必须外化为可见事件
- ❌ 用 `wc -c`（字节数）统计中文字数——用 Python 字符统计或 `wc -m`
- ❌ 锁定后未经用户确认就改前 10 章细纲
- ❌ 写完不更新 `追踪/伏笔.md`、`时间线.md`、`角色状态.md`
